...
class Program
{
    private static Configuration _configuration = GetConfigurationSample1();
    private static AgentDef agentDef = new AgentDef();

    static void Main(string[] args)
    {
        AgentDef agentDef = _configuration.AgentDefs.First(x => x.Id == "default");
        Hagent hagent = agentDef.GetHagent(_configuration);
        var response = hagent.Query(new ChatQuery() { Query = "dime algo sobre ti" });
